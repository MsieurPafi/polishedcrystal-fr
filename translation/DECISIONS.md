...
| B3 | **Vocabulaire moderne ou d'époque ?** | ⚠️ **Mesure faite : le hack modernise systématiquement.** Il a converti tout l'ALL-CAPS vanilla en casse mixte (1 311 blocs). Le français devra suivre — donc `ECORCIA` devient `Écorcia`… mais la Gen 2 française n'accentue pas les capitales ([`CONVENTIONS.md`](CONVENTIONS.md) §2), et en casse mixte la question ne se pose plus pareil. **À trancher avec l'utilisateur.** |
