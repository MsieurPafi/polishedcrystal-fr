...
|---|---|---|
| A1 | **Quelles cases de charmap sacrifier ?** Il en faut **18**, et le plafond est dur : **125 jetons compressibles**, tous occupés. | ⚠️ **Arithmétique corrigée le 2026-08-24.** J'avais annoncé que libérer l'espagnol comblerait le manque : c'est faux. Besoin 18 (9 accentuées `à â ç è ê î ô ù û` + 9 élisions) ; libérables sans arbitrage : 6 contractions anglaises + `É` = **7**. Avec l'espagnol : **12**. **Il manque encore 6 cases.** Pistes non encore mesurées : déplacer un caractère compressible rare (`♥`, `♪`, `×`, `%`) vers la zone NON compressible `$ec-$ff` pour libérer sa feuille ; y récupérer `′` et `″` si le Pokédex passe au métrique. `ï` est abandonné (6 occurrences dans tout le Cristal français). |
| A2 | **Le PNJ hispanophone `SIGHTSEERF PILAR` de `maps/ShamoutiCoast.asm`** — seul utilisateur de `á í ó ¿ ¡` dans tout PC. | **Décision éditoriale de l'utilisateur.** ⚠️ Libérer ces 5 cases **ne suffit pas** à combler le manque (voir A1) — l'arbitrage se joue donc sur le fond, pas sur la place. Précédent officiel trouvé : voir [`GLOSSAIRE.md`](GLOSSAIRE.md) §5. |
